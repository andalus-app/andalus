...
const SCROLL_NUDGE_THRESHOLD = 5;
const GPS_PROMPT_KEY = 'gps-prompt-shown';
...
function Shell() {
  ...
  const [tab, setTab] = useState(() => { try { return sessionStorage.getItem('activeTab') || 'home'; } catch { return 'home'; } });
  ...
  useEffect(() => {
    ...
    if (el) { el.scrollTop = 0; requestAnimationFrame(() => { if (el) el.scrollTop = 0; }); }
  ...
  useEffect(() => {
    ...
    const restoreHandler = (e) => { if (scrollContainerRef.current) scrollContainerRef.current.scrollTop = e.detail?.top || 0; };
    window.addEventListener('scrollToTop', toTopHandler);
    window.addEventListener('restoreScroll', restoreHandler);
    return () => { window.removeEventListener('scrollToTop', toTopHandler); window.removeEventListener('restoreScroll', restoreHandler); };
  }, []);

  // GPS — always fetch on open, no distance threshold, uses Permissions API
  ...
  useEffect(() => {
    ...
    let timer = null;
    const doFetch = () => {
      try { const stored = JSON.parse(localStorage.getItem('bonetiderState') || '{}'); if (stored.settings?.autoLocation === false) return; } catch {}
    ...
    const checkLocation = async () => {
      ...
      if (navigator.permissions) {
        try {
          ...
          if (perm.state === 'denied') { window.dispatchEvent(new CustomEvent('gps-denied')); return; }
  ...
  // Nudge — first visit + every 7 days, scroll to reveal 6th tab
  useEffect(() => {
    if (TABS.length <= SCROLL_NUDGE_THRESHOLD) return;
    const SEVEN_DAYS_MS = 7 * 24 * 60 * 60 * 1000;
    try { const last = parseInt(localStorage.getItem(nudgeDoneKey) || '0', 10); if (Date.now() - last < SEVEN_DAYS_MS) return; } catch {}
    const t = setTimeout(() => {
      setNudging(true);
  ...
  const [bookingStartView, setBookingStartView] = useState(null);

  const handleTabPress = (id) => {
    showTabBar(); setTabBarVisible(true);
    if (id === 'booking') {
      setHighlightBookingId(null); setHighlightFilter(null);
      setAdminHighlightId(null); setAdminHighlightFilter(null);
      setAdminInitialFilter(null); setBookingStartView(null);
      setBookingRefreshKey(k => k + 1);
    }
    if (id === 'ebooks') {
      if (tab === 'ebooks') { setEbooksReset(n => n + 1); } else { setTab('ebooks'); setEbooksReset(n => n + 1); }
      setShowMonthly(false); try { sessionStorage.setItem('activeTab', 'ebooks'); } catch {} return;
    }
    if (id === 'more') { setMoreInitialView(null); setMoreResetKey(n => n + 1); }
    setTab(id); setShowMonthly(false);
  ...
  const handleGoToMyBookings = (bookingId, status) => {
    ...
    setTab('booking'); try { sessionStorage.setItem('activeTab', 'booking'); } catch {}
  };

  const handleGoToAdminLogin = () => {
    setAdminInitialFilter(null); setBookingStartView('admin');
    setTab('booking'); try { sessionStorage.setItem('activeTab', 'booking'); } catch {}
  };

  const handleGoToCancelledBookings = (highlightId = null) => {
    setAdminHighlightFilter(null); setAdminHighlightId(null); setAdminInitialFilter(null); setBookingStartView(null);
    setTimeout(() => {
      setAdminInitialFilter('cancelled'); setAdminHighlightFilter('cancelled');
      setAdminHighlightId(highlightId || cancelledBookingIds[0] || null); setBookingStartView('admin');
    }, 0);
    setTab('booking'); try { sessionStorage.setItem('activeTab', 'booking'); } catch {}
  };
  ...
  const handleGoToPendingBookings = () => {
    setAdminHighlightFilter(null); setAdminHighlightId(null); setAdminInitialFilter(null); setBookingStartView(null);
    setTimeout(() => {
      setAdminInitialFilter('pending'); setAdminHighlightFilter('pending');
      setAdminHighlightId(pendingBookingIds?.[0] || null); setBookingStartView('admin');
    }, 0);
    setTab('booking'); try { sessionStorage.setItem('activeTab', 'booking'); } catch {}
  };
  ...
  const handleTopTap = useCallback((e) => { if (e.clientY <= 44) scrollContainerRef.current?.scrollTo({ top: 0, behavior: 'smooth' }); }, []);
  const swipeRef = useRef(null);
  const handleTouchStart = useCallback((e) => { const t = e.touches[0]; if (t.clientX < 28) swipeRef.current = { x: t.clientX, y: t.clientY }; else swipeRef.current = null; }, []);
  const handleTouchEnd = useCallback((e) => {
    if (!swipeRef.current) return;
    const t = e.changedTouches[0]; const dx = t.clientX - swipeRef.current.x; const dy = Math.abs(t.clientY - swipeRef.current.y);
    if (dx > 60 && dy < 80) window.dispatchEvent(new CustomEvent('edgeSwipeBack'));
    swipeRef.current = null;
  }, []);

  return (
    <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onClick={handleTopTap}
      ...
      <div style={{
        ...
        left: '50%',
        transform: effectiveTabBarVisible ? 'translateX(-50%) translateY(0)' : 'translateX(-50%) translateY(calc(100% + 24px))',
        ...
        borderRadius: 28, border: `1px solid ${T.border}`,
        ...
        {/* Scrollable tab row — all 6 tabs, nudge scrolls to reveal 6th */}
        <div ref={tabScrollRef} className="tab-scroll" style={{
              ...
              <button key={t.id} onClick={() => handleTabPress(t.id)} style={{
                ...
                {t.type === 'custom' ? (
                  <div style={{ position: 'relative', display: 'inline-flex' }}>
                    {t.icon === 'booking' ? (
                      <CalendarClockIcon size={22} color={active ? T.accent : T.isDark ? T.accent : T.text} style={{ opacity: active ? 1 : T.isDark ? 0.75 : 1, transition: 'all .2s' }} />
                    ) : t.icon === 'ebooks' ? (
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? T.accent : T.isDark ? T.accent : T.text} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: active ? 1 : T.isDark ? 0.75 : 1, transition: 'all .2s' }}>
                        <path d="M2 6s2-2 5-2 5 2 5 2v14s-2-1-5-1-5 1-5 1V6z"/>
                        <path d="M12 6s2-2 5-2 5 2 5 2v14s-2-1-5-1-5 1-5 1V6z"/>
                      </svg>
                    ) : (
                      <img src={t.icon === 'kaba' ? KabaIcon : t.icon === 'more' ? MoreAppIcon : PrayerTimesIcon} alt={t.label}
                        style={{ width: 24, height: 24, objectFit: 'contain', filter: active ? svgColorFilter(T.isDark) : T.isDark ? 'invert(48%) sepia(60%) saturate(400%) hue-rotate(120deg) brightness(90%)' : 'none', transition: 'filter .2s' }} />
                    )}
                    {t.id === 'booking' && (visitorUnread > 0 || adminPendingCount > 0 || cancelledUnread > 0) && (
                      <div style={{ position: 'absolute', top: -3, right: -4, display: 'flex', gap: 2 }}>
                        {adminPendingCount > 0 && <div style={{ minWidth: 14, height: 14, borderRadius: 7, background: '#f59e0b', color: '#fff', fontSize: 8, fontWeight: 800, fontFamily: 'system-ui', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px', boxSizing: 'border-box', border: `1.5px solid ${T.isDark ? 'rgba(18,18,18,0.9)' : 'rgba(245,248,247,0.9)'}` }}>{adminPendingCount > 9 ? '9+' : adminPendingCount}</div>}
                        {visitorUnread > 0 && <div style={{ minWidth: 14, height: 14, borderRadius: 7, background: '#ef4444', color: '#fff', fontSize: 8, fontWeight: 800, fontFamily: 'system-ui', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px', boxSizing: 'border-box', border: `1.5px solid ${T.isDark ? 'rgba(18,18,18,0.9)' : 'rgba(245,248,247,0.9)'}` }}>{visitorUnread > 9 ? '9+' : visitorUnread}</div>}
                        {cancelledUnread > 0 && <div onClick={e => { e.stopPropagation(); if (navigator.vibrate) navigator.vibrate([60,40,60,40,120]); handleGoToCancelledBookings(); }} style={{ minWidth: 14, height: 14, borderRadius: 7, background: '#3b82f6', color: '#fff', fontSize: 8, fontWeight: 800, fontFamily: 'system-ui', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px', boxSizing: 'border-box', border: `1.5px solid ${T.isDark ? 'rgba(18,18,18,0.9)' : 'rgba(245,248,247,0.9)'}`, cursor: 'pointer' }}>{cancelledUnread > 9 ? '9+' : cancelledUnread}</div>}
                      </div>
                    )}
                  </div>
                ) : (
                  <div style={{ position: 'relative', display: 'inline-flex' }}>
                    <SvgIcon name={t.iconName} size={22} color={active ? T.accent : T.isDark ? T.accent : T.text} style={{ opacity: active ? 1 : T.isDark ? 0.75 : 1, transition: 'all .2s' }} />
                    {t.id === 'home' && isLive && <div style={{ position: 'absolute', top: -3, right: -4, width: 11, height: 11, borderRadius: '50%', background: '#FF0000', border: `2px solid ${T.isDark ? 'rgba(18,18,18,0.95)' : 'rgba(245,248,247,0.95)'}`, animation: 'liveDot 1s ease-in-out infinite, liveRing 1.5s ease-out infinite' }} />}
                    {t.id === 'home' && isUpcoming && !isLive && <div style={{ position: 'absolute', top: -3, right: -4, width: 9, height: 9, borderRadius: '50%', background: '#f59e0b', border: `2px solid ${T.isDark ? 'rgba(18,18,18,0.95)' : 'rgba(245,248,247,0.95)'}` }} />}
                  </div>
                )}
                <span style={{ fontSize: 9, fontWeight: active ? 600 : 500, letterSpacing: '.3px', color: t.id === 'home' && isLive ? '#FF0000' : t.id === 'home' && isUpcoming ? '#f59e0b' : active ? T.accent : T.isDark ? T.accent : T.text, opacity: active ? 1 : T.isDark ? 0.7 : 1, whiteSpace: 'nowrap', fontFamily: "'Inter',system-ui,sans-serif", transition: 'all .2s' }}>
                  {t.id === 'home' && isLive ? 'LIVE' : t.id === 'home' && isUpcoming ? 'Snart' : t.label}
                </span>
              </button>
            );
          })}
        ...
        {/* Peek-fade — always visible, hints that Visa mer exists to the right */}
        {TABS.length > SCROLL_NUDGE_THRESHOLD && (
          <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 32, background: T.isDark ? 'linear-gradient(to right, transparent, rgba(18,18,18,0.75))' : 'linear-gradient(to right, transparent, rgba(245,248,247,0.75))', borderRadius: '0 28px 28px 0', pointerEvents: 'none' }} />
        )}
      </div>
...
const PARTICLES = [
  { angle: 0, dist: 52 }, { angle: 45, dist: 48 }, { angle: 90, dist: 52 }, { angle: 135, dist: 48 },
  { angle: 180, dist: 52 }, { angle: 225, dist: 48 }, { angle: 270, dist: 52 }, { angle: 315, dist: 48 },
];

function SplashScreen({ onDone }) {
  useEffect(() => { const t = setTimeout(onDone, 1100); return () => clearTimeout(t); }, [onDone]);
  const isDark = (() => { try { const s = localStorage.getItem('theme'); if (s === 'dark') return true; if (s === 'light') return false; } catch {} return window.matchMedia?.('(prefers-color-scheme: dark)').matches ?? true; })();
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: splashBg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', animation: 'splashOut 0.35s 0.85s cubic-bezier(0.76,0,0.24,1) both' }}>
      <style>{`
        @keyframes splashLogoIn{from{transform:scale(0) rotate(-20deg);opacity:0}to{transform:scale(1) rotate(0deg);opacity:1}}
        @keyframes splashParticle{0%{opacity:0.9;transform:translate(0,0) scale(1.2)}100%{opacity:0;transform:var(--pt) scale(0)}}
        @keyframes splashName{from{opacity:0;letter-spacing:12px}to{opacity:1;letter-spacing:7px}}
      ...
      <div style={{ position: 'relative', width: 40, height: 40 }}>
        {PARTICLES.map((p, i) => {
          ...
          return <div key={i} style={{ position: 'absolute', top: '50%', left: '50%', width: 5, height: 5, marginTop: -2.5, marginLeft: -2.5, borderRadius: '50%', background: '#2D8B78', '--pt': `translate(${tx}px,${ty}px)`, animation: `splashParticle 0.65s ${0.05 + i * 0.02}s cubic-bezier(0.2,0,1,1) both` }} />;
        })}
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'splashLogoIn 0.5s 0.1s cubic-bezier(0.34,1.56,0.64,1) both' }}>
      ...
      <div style={{ marginTop: 20, fontSize: 13, fontFamily: "'Inter',system-ui,sans-serif", fontWeight: 500, letterSpacing: 3, color: textColor, textTransform: 'uppercase', animation: 'splashName 0.5s 0.45s cubic-bezier(0.16,1,0.3,1) both' }}>
...
export default function App() {
  const [showSplash, setShowSplash] = React.useState(() => {
    try { if (sessionStorage.getItem('andalus_session_started')) return false; sessionStorage.setItem('andalus_session_started', '1'); return true; } catch { return false; }
  });
  return (
    <ThemeProvider>
      <AppProvider>
